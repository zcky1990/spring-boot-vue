...
export default class CodeBlockCommand extends Command {
	...
	_applyQuote( writer, blocks ) {
		...
		getRangesOfBlockGroups( writer, blocks ).reverse().forEach( groupRange => {
			...
			if ( !quote ) {
				quote = writer.createElement( 'pre' );
...
function findQuote( elementOrPosition ) {
	return elementOrPosition.parent.name == 'pre' ? elementOrPosition.parent : null;
}
...
function checkCanBeQuoted( schema, block ) {
	...
	const isBQAllowed = schema.checkChild( block.parent, 'pre' );
	const isBlockAllowedInBQ = schema.checkChild( [ '$root', 'pre' ], block );
